...
ベンチマーカーが応答を見て次の利用者やグループを開始する場合、ある API の短縮後に
同じ時間内で扱うグループ数や人数分布が変わることがある。各グループで必ず行う
作成・終了処理は、人数の少ないグループが増えると、有効な要求あたりの負担が大きくなる。

スコアや対象 API の平均だけでなく、負荷開始から序盤・中盤・後半の成功数と遅延、
同時利用者数、開始・終了したグループ数、人数分布、グループごとの固定 API の件数と
累積時間をそろえて比較する。局所的な SQL 短縮と、後半に増えた別 API の仕事量は分けて読む。
グループ構成の差が変更によるものか試行間の揺らぎかは単発計測では決められないため、
可能なら無変更の再測定や反復比較で確かめる。
