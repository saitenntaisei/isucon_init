...
DB 更新後にキャッシュの件数などを加減算する方式では、古い読み取り結果だけでなく、
**DB の新しい値が差分適用より先に格納される競合**にも注意する。
たとえば件数を 10 から 9 に減らす DB 更新が完了し、同時の cache miss が 9 を格納してから
更新側が `-1` を適用すると、キャッシュは誤った 8 になる。更新完了時だけ世代を進めても防げない。

この方式は、対象キーに影響する全更新経路を同じ仕組みで管理できる場合に使う。
対象キーの DB 更新前に世代を進め、処理中の writer 数を増やす。読み取り開始から格納までに
writer が存在した、または世代が変わった結果は格納しない。commit 成功後は更新前から残る
エントリだけに差分を適用し、writer 数を戻して世代を進めてから応答する。並行更新には boolean
ではなく件数を使い、DB 待ちの間はキャッシュの mutex を保持しない。失敗や結果不明時は無効化する。
初期化でキャッシュを全消去した後に、初期化前の完了 callback が新しい状態へ差分を適用しないようにする。

再試行可能な状態変更では、直前の SELECT だけで差分の適用を決めない。条件付き UPDATE と
driver の affected-row semantics を確認し、実際に変更した行数が確認できた場合だけ加減算する。
同じ行への並行リクエストでは、DB 更新が 1 回でも両方が古い状態を読めるためである。
競合の順序をソース上で追い、ベンチマーカーの整合性検証も確認する。1 回の成功だけで競合不在とは判断しない。
